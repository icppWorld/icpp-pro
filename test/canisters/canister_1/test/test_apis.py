...
def test__roundtrip_text_to_json_to_text__long_message(
    ...
    # icp-cli wraps a long response over several lines, so flatten it first
...
def test__roundtrip_record(network: str, principal: str) -> None:
    ...
    # icp-cli pretty-prints records over several lines, so flatten it as well.
    ...
    assert flatten_candid_text(response).replace('"', "") == expected_response.replace(
        '"', ""
    )
